...
#bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t%QUAL\t%AC\t%AF\t%LV\n' -r chr19 -o chr19.info [relevantvcf] 
...
colnames(myd)=c('CHROM','POS','REF','ALT','QUAL','AC','AF', 'LV' , 'PS'  )

p<- myd %>% select (CHROM, POS , AC) %>% separate_rows(AC , convert=T ) %>% group_by(CHROM, POS  )%>% summarise (ALTsum=sum(AC)) 
p1<- ggplot(p, aes(ALTsum ) )+geom_bar() + theme_minimal()  + ggtitle(paste('counts ', opt$experiment , sep='-'))
p2<- ggplot(p, aes(ALTsum ) )+geom_bar(aes(y=..prop..)) + theme_minimal()  + ggtitle(paste('fraction ', opt$experiment , sep='-'))
p3<-  ggplot(p, aes(ALTsum ) )+geom_bar() + theme_minimal() +  
	#+xlim (0,50) +
	ggtitle(paste('counts ', opt$experiment , sep='-'))
p4<-  ggplot(p, aes(ALTsum ) )+geom_bar() + theme_minimal()   +
	#xlim(50,100 ) +
	ggtitle (paste('counts ', opt$experiment , sep='-'))

g<-grid.arrange( p1, p2, nrow=2 )  # p3, p4, nrow=4 )
ggsave(paste('sfscounts.', opt$experiment, '.pdf', sep=''), g  ) 
...
ggsave(paste('sfsfreq' , opt$experiment,  '.pdf', sep='' ) ) 
